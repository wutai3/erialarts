...
sub leteng_resform
{
	...
	print qq!<a href="javascript:view('NO$ATC->{number}');">レス欄生成▼</a>! if(defined($FORM{dhtml}));
	print "</div>\n</div>\n\n";
	...
	# Normal mode
		...
		print qq!\t<table summary="res"><tbody>\n!;
		print qq!\t<tr>\n!;
		print qq!\t\t<td><label class="requisite">Name<input type=text name="name" size=8 value="$COOKIE{name}" class="in"></label>\n!;

		print qq!\t\t　key<input type=text name="pwd" size=8 maxlength=8 value="$COOKIE{pwd}" class="in" style="ime-mode:disabled">\n!;
		print "\t\t　color@{[ $sozai->irolist() ]}\n";
		print "\t\t　@{[ $sozai->icolist() ]}</td>\n";

		print qq!\t\t<td class="submit"><input type=submit value=" 返信 " class="min"></td></tr>\n!;

		print qq!\t\t<tr><td colspan=2><textarea cols=68 rows=3 name="comment"></textarea></td>\n!;
